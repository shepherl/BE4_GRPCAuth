...
def run():
    ...
    try:
        ...
        reg_response = stub.Register(auth_pb2.RegisterRequest(
            email=test_email,
            first_name="Иван",
            last_name="Иванов",
            phone="+79991234567",
        ...
        val_response = stub.ValidateToken(auth_pb2.ValidateTokenRequest(token=real_token))
        print(f"   ✅ Запрос успешен! Результат проверки (is_valid): {val_response.is_valid}")
    ...
    try:
        ...
        if e.code() == grpc.StatusCode.UNAUTHENTICATED:
            print(f"   ✅ Успех! Сервер отклонил подделку. Статус: {e.code()} — '{e.details()}'")
        else:
            print(f"   ⚠️ Сервер вернул неожиданную ошибку: {e.code()} — '{e.details()}'")
